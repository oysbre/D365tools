...
#Install/update PSmodule SQLserver
if((Get-Module sqlserver -ListAvailable) -eq $null){
    Write-host "Installing PSmodule sqlserver..." -foregroundcolor yellow
    Install-Module sqlserver -Force -AllowClobber
}
else {
	$sqlserverPSmod = Get-Module sqlserver -ListAvailable
	if ($sqlserverPSmod.Version.Major -lt 22){
...
Clear-D365BacpacTableData -Path $sqlbakPath -Table "SECURITYOBJECTHISTORY","*Staging*","LACFAILOVERPREVIEW","BATCHHISTORY","BatchJobHistory","SYSDATABASELOG*","ReqCalcTaskTrace","LACARCHIVE*","BISWSHISTORY","DTA_*","BISMESSAGEHISTORYHEADER","RETAILTRANSACTIONPAYMENTTRANS","SRSTMPDATASTORE","MCRORDEREVENTTABLE","EVENTCUDLINES","EVENTINBOXDATA","EVENTINBOX","RETAILEODSTATEMENTCONTROLLERLOG","SMMTRANSLOG","EO_InventoryToDDDLog","EO_DimensionsLog","COSTSHEETCACHE","SALESQUOTATIONPARMTABLE","SALESQUOTATIONPARMLINE","BOMCALCTRANS","CGISERVICEBUSMESSAGELOG","FRATSTIMESHEETLINESTAGING","AIFCHANGETRACKINGDELETEDOBJECT","TSTIMESHEETLINELOG","JMGCHANGELOG" -ClearFromSource -ErrorAction SilentlyContinue
write-host
...
& "$bacpacexepath\SqlPackage.exe" /a:import /sf:$sqlbakPath /tsn:localhost /tdn:$newDBname /p:CommandTimeout=0 /p:DisableIndexesForDataPhase=FALSE /ttsc:True /mfp:"$($localdir)BacpacModel-edited.xml" /DiagnosticsFile:"$($localdir)RestoreLog_$($datestamp).log" /DiagnosticsLevel:Verbose
